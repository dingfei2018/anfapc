...
function getData (){
    $.ajax({
        url : '/kd/finance/receivable/getGoodsPayReJson',
        data : {
...
function confirm() {
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/receivable/confirmGoodsPay?shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/receivable/goodsPayIndex';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
