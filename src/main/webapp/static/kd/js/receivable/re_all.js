...
function ReAll(){
    ...
    ReAll.prototype={
		...
		getData:function(param, pageNo, obj){
			...
			$.ajax({
				type:'post',
				url:'/kd/finance/receivable/getAllShipReJson',
		...
		appendHtml:function(obj,index){
			var html="";
			var feeState=obj.state==1?"已结算":obj.state==2?"未结算":"部分结算";
        	...
        	html+='<tr>' +
                    ...
                    '<td style="color:orange;">'+obj.ship_total_fee+'</td>'+
                    '<td style="color:orange;">'+obj.not_total_fee+'</td>'+
					'<td>'+obj.ship_nowpay_fee+'</td>'+
					'<td>'+obj.not_nowpay_fee+'</td>'+
					'<td>'+obj.ship_pickuppay_fee+'</td>'+
					'<td>'+obj.not_pickuppay_fee+'</td>'+
					'<td>'+obj.ship_receiptpay_fee+'</td>'+
					'<td>'+obj.not_receiptpay_fee+'</td>'+
					'<td>'+obj.ship_monthpay_fee+'</td>'+
					'<td>'+obj.not_monthpay_fee+'</td>'+
					'<td>'+obj.ship_arrearspay_fee+'</td>'+
					'<td>'+obj.not_arrearspay_fee+'</td>'+
					'<td>'+obj.ship_goodspay_fee+'</td>'+
					'<td>'+obj.not_goodspay_fee+'</td>'+
		...
		excelExport:function(){
			...
			layer.confirm('您确定要导出excel？', {
    			...
    			window.location.href="/kd/finance/receivable/downExcelAll?"+obj.searchParam;
    			layer.close(index);
    		}, function(){});
		}
};
