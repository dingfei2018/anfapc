...
function getData (){
    $.ajax({
        url : '/kd/finance/receivable/getNowPayReJson',
        data : {
...
function confirm() {
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/receivable/confirmNowPay?shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/receivable/nowpayIndex';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
