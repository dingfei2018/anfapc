...
function getData (){
    $.ajax({
        url : '/kd/finance/receivable/getPickupPayReJson',
        data : {
            receiverId : $('#receiverName').val(),
            networkId:$('#netWorkId').val(),
            toNetworkId:$('#toNetWorkId').val(),
            flag:'all'
        },
        async:false,
        type : 'post',
...
function confirm() {
    ...
    var totalFee=0;
    $(tr).each(function(){
            shipIds.push($(this)[0].children[12].children[0].value);
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/receivable/confirmPickupPay?shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/receivable/pickupPayIndex';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
