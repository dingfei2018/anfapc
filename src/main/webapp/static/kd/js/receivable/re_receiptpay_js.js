...
function getData (){
    $.ajax({
        url : '/kd/finance/receivable/getReceiptPayReJson',
        data : {
                ...
                for (var i = 0; i < row.length; i++) {
                    if(checkShipId(row[i].ship_id)) {
                        html += '<tr class="tr_css" align="center" ondblclick="doubleClickConfirm($(this));">'
                            ...
                            + '<td>' + row[i].ship_receiptpay_fee + '</td>'
...
function confirm() {
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/receivable/confirmReceiptPay?shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/receivable/receiptPayIndex';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
