...
ShipLoading.prototype={
		...
		loadnetworkchange:function(){
			this.toAddrCombobox(2, $("#load_next_network_id").val(), this);
		},
		...
		search:function(){
			var param ="type=3&endCode="+$("#endCode").val()+"&networkId="+$("#networkId").val()+"&pageNo=1";
		...
		fahuo:function(){
			...
			sdata +="&load_transport_type=3&ids="+this.selectIds.join();
			$.ajax({
				type:'POST',
				url:'/kd/loading/up',
				data:sdata,
		...
		validateData:function(){
			...
			if($("#load_next_network_id").val()==$("#network_id").val()){
				layer.msg("配载网点和到货网点不能相同");
				return false;
			}
