...
function updateabel() {
    var trunkLineType =$('#trunkLineType').val();
        document.getElementById("trunkLineType"+trunkLineType).href="#";
        document.getElementById("trunkLineType"+trunkLineType).className ="activet at";
...
function getData (){
    $.ajax({
        url : '/kd/finance/payable/trunkLoadSearch?fillStatus=0',
        data : {
            truckNumber : $('#truckNumber').val(),
            driverName:$('#driverName').val(),
            trunkLineType:$('#trunkLineType').val(),
            networkId:0,
            networkId:0,
        ...
        success : function(result) {
            if (result) {
                var row=result.loads.list;
                var trunkLineType = result.trunkLineType;
                var fee = 0 ;
                var html="";
                for (var i = 0; i < row.length; i++) {
                    if(checkShipId(row[i].load_id)) {
                        ...
                        if(trunkLineType==1) fee = row[i].load_nowtrans_fee;
                        else if(trunkLineType==2) fee = row[i].load_nowoil_fee;
                        else if(trunkLineType==3) fee = row[i].load_backtrans_fee;
                        else if(trunkLineType==4) fee = row[i].load_allsafe_fee;
                        else if(trunkLineType==5) fee = row[i].load_start_fee;
                        else if(trunkLineType==6) fee = row[i].load_other_fee;
                        else if(trunkLineType==7) fee = row[i].load_attrans_fee;
                        else if(trunkLineType==8) fee = row[i].load_atunload_fee;
                        else if(trunkLineType==9) fee = row[i].load_atother_fee;
                        html += '<tr class="tr_css" align="center" ondblclick="doubleClickConfirm($(this));">'
                            ...
                            + '<td>' + row[i].enetworkName + '</td>'

                            + '<td>' + fee + '</td>'

                            + '<td>' + row[i].truck_id_number + '</td>'
                            + '<td>' + row[i].truck_driver_name + '</td>'
                            + '<td>' + row[i].load_depart_time  + '<input type="hidden" value="' + row[i].load_id + '"></td>'
                            + '</tr>';
                    }
                }
...
function confirm() {
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/payable/trunkSettlementLoads?loadIds='+loadIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("确认成功！",{time: 1000},function(){
                            if(trunkLineType == 8) ustr="?loadAtFeeFlag=0"
                            else if(trunkLineType == 9) ustr="?loadAtFeeFlag=1"
                            else ustr="?transportType=3&trunkLineType="+trunkLineType
                            window.location.href='/kd/finance/payable/loadTransportPage'+ustr;
                        });
                    }else{
                        layer.msg("确认失败！");
                    }
