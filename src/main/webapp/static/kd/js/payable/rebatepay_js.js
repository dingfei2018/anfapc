...
function getData (){
    $.ajax({
        url : '/kd/finance/payable/getRebateSettlement',
        data : {
            senderId : $('#shipperName').val(),
            shipNetWorkId:$('#netWorkId').val(),
        },
        async:false,
        type : 'post',
        dataType : 'json',
                ...
                for (var i = 0; i < row.length; i++) {
                    ...
                    if(checkShipId(row[i].ship_id)) {
                        html += '<tr class="tr_css" align="center" ondblclick="doubleClickConfirm($(this));">'
                            ...
                            + '<td>' + row[i].toCity+ '<input type="hidden" value="' + row[i].ship_id + '"></td>'
                            + '</tr>';
                    }
                }
...
function confirm() {
    ...
    $(tr).each(function(){
            ...
            totalFee+=parseFloat($(this)[0].children[7].innerText);
    });
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/payable/confirmRebate?shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/payable/rebate';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
