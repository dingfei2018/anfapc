...
function getData (){
    $.ajax({
        url : '/kd/finance/account/getReDetailedAccountList',
        data : {
...
function confirm() {
    ...
    layer.confirm(
        ...
        function() {
            $.ajax({
                type:'POST',
                url:'/kd/finance/account/confirmReDetailed?feeIds='+feeIds+'&shipIds='+shipIds,
                data:$('#confirmform').serialize(),
                success:function(data){
                    if(data.success){
                        layer.msg("结算成功！",{time: 1000},function(){
                            window.location.href='/kd/finance/account/reDetailedIndex';
                        });
                    }else{
                        layer.msg("结算失败！");
                    }
