...
function toAddrCombobox(netWorkId){
    ...
    $('#toAddr').combobox({
        ...
        textField: 'toAdd',
        onSelect: function (row) {
            $('#toNetWorkName').val(row.arriveNetworkName);
            $('#toNetWorkId').val(row.arrive_network_id);
            $('#toCode').val(row.to_city_code);
            $('#rcode').val(row.to_city_code);
            var code = row.to_city_code;
...
function changeNameForProduct(){
    ...
    var pname2= $("input[name='kdProduct[1].product_name']").val();
    var pname3=$("input[name='kdProduct[2].product_name']").val();
    console.log(pname3);
