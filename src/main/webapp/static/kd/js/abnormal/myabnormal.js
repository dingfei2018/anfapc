...
Abnormal.prototype={
		...
		getData:function(param, pageNo, obj){
			...
			$.ajax({
				type:'post',
				url:'/kd/abnormal/getAbnormalPageList?flag=my',
		...
		appendHtml:function(obj, index){
			...
			var type=obj.abnormal_type==0?'货损':obj.abnormal_status==1?'少货':obj.abnormal_status==2?'多货':
				obj.abnormal_status==3?'货物丢失':obj.abnormal_status==4?'货单不符':obj.abnormal_status==5?'超重超方':
				obj.abnormal_status==6?'超时':obj.abnormal_status==7?'拒收':obj.abnormal_status==8?'投诉':'其他';
		...
		excelExport:function(){
			...
			layer.confirm('您确定要导出excel？', {
    			...
    			window.location.href="/kd/abnormal/exportMyAbnormallist?flag=my&"+obj.searchParam;
    			layer.close(index);
    		}, function(){});
		}
