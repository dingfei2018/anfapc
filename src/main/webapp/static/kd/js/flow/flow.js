...
Payable.prototype={
		...
		appendHtml:function(obj, index){
			...
			html += "<td>"+index+"</td>";
			html += "<td class='viewId' data='"+obj.flow_sn+"' style='color: #3974f8;cursor: pointer;'>"+obj.flow_sn+"</td>";
            html += "<td>"+obj.networdName+"</td>";
            ...
            html += "<td>"+((obj.remark==null)?"" : obj.remark)+"</td>";
		...
		openDetail:function(){
			var flowSn = $(this).attr("data");
            window.location.href="/kd/finance/flow/detail?flowSn="+flowSn+"&type=open";
		},
		excelExport:function(){
			...
			layer.confirm('您确定要导出excel？', {
    			...
    			window.location.href="/kd/finance/flow/downFlow?"+obj.searchParam;
    			layer.close(index);
    		}, function(){});
		}
};
