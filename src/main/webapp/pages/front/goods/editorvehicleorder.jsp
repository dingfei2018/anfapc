...
<body>
	...
	<%@ include file="../common/loginhead.jsp" %>
		...
		<input type="hidden" id="orderid" name="orderid" value="${Linesdata.id}"> 
                ...
                <p><b class="benner-opsan">线路信息</b><span>* 必填</span>
					...
					<div id="location" class="citys">
						<span>*</span>&nbsp;&nbsp;&nbsp;发货地
							 ...
							 <option >${fromaddr.province}</option>
							 </select>
		                     <select id="city" name="scity">
		                     <option >${fromaddr.city}</option>
			...
			<div class="benner-translatet">
				...
				<table id="questionTable" class="benner-translatet-table">
					...
					<tr>
						...
						<td>
							<input name="Ordergoods[${v.index}].amount" type="text" datatype="sl" value="${item.amount}"  onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')"/> 件
						</td>
						<td>
							<input type="text" name="Ordergoods[${v.index}].weight"  datatype="zl" value="${item.weight}"  onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')"/>
            ...
            <div class="benner-translateo">
            	<p><b class="benner-opsan">需求说明</b></p>
            	<span>车辆需求：</span>
            	
            	<select id="truck_length" name="model.truck_length">
            		<option value="1" <c:if test="${item.truck_length eq 1}">selected="selected"</c:if>>3.8米</option>
            		<option value="2" <c:if test="${item.truck_length eq 2}">selected="selected"</c:if>>4米</option>
            		<option value="3" <c:if test="${item.truck_length eq 3}">selected="selected"</c:if>>4.2米</option>
            		<option value="4" <c:if test="${item.truck_length eq 4}">selected="selected"</c:if>>4.8米</option>
            		<option value="5" <c:if test="${item.truck_length eq 5}">selected="selected"</c:if>>5.2米</option>
            		<option value="6" <c:if test="${item.truck_length eq 6}">selected="selected"</c:if>>5.8米</option>
            		<option value="7" <c:if test="${item.truck_length eq 7}">selected="selected"</c:if>>6.2米</option>
            		<option value="8" <c:if test="${item.truck_length eq 8}">selected="selected"</c:if>>6.8米</option>
            		<option value="9" <c:if test="${item.truck_length eq 9}">selected="selected"</c:if>>7.2米</option>
            		<option value="10" <c:if test="${item.truck_length eq 10}">selected="selected"</c:if>>7.6米</option>
            		<option value="11" <c:if test="${item.truck_length eq 11}">selected="selected"</c:if>>7.8米</option>
            		<option value="12" <c:if test="${item.truck_length eq 12}">selected="selected"</c:if>>8米</option>
            		<option value="13" <c:if test="${item.truck_length eq 13}">selected="selected"</c:if>>8.6米</option>
            		<option value="14" <c:if test="${item.truck_length eq 14}">selected="selected"</c:if>>8.7米</option>
            		<option value="15" <c:if test="${item.truck_length eq 15}">selected="selected"</c:if>>8.8米</option>
            		<option value="16" <c:if test="${item.truck_length eq 16}">selected="selected"</c:if>>9米</option>
            		<option value="17" <c:if test="${item.truck_length eq 17}">selected="selected"</c:if>>9.6米</option>
            		<option value="18" <c:if test="${item.truck_length eq 18}">selected="selected"</c:if>>9.8米</option>
            		<option value="19" <c:if test="${item.truck_length eq 19}">selected="selected"</c:if>>10米</option>
            		<option value="20" <c:if test="${item.truck_length eq 20}">selected="selected"</c:if>>10.6米</option>
            		<option value="21" <c:if test="${item.truck_length eq 21}">selected="selected"</c:if>>11米</option>
            		<option value="22" <c:if test="${item.truck_length eq 22}">selected="selected"</c:if>>12.5米</option>
            		<option value="23" <c:if test="${item.truck_length eq 23}">selected="selected"</c:if>>13米</option>
            		<option value="24" <c:if test="${item.truck_length eq 24}">selected="selected"</c:if>>13.5米</option>
            		<option value="25" <c:if test="${item.truck_length eq 25}">selected="selected"</c:if>>14米</option>
            		<option value="26" <c:if test="${item.truck_length eq 26}">selected="selected"</c:if>>14.5米</option>
            		<option value="27" <c:if test="${item.truck_length eq 27}">selected="selected"</c:if>>15米</option>
            		<option value="28" <c:if test="${item.truck_length eq 28}">selected="selected"</c:if>>15.5米</option>
            		<option value="29" <c:if test="${item.truck_length eq 29}">selected="selected"</c:if>>16米</option>
            		<option value="30" <c:if test="${item.truck_length eq 30}">selected="selected"</c:if>>16.5米</option>
            		<option value="31" <c:if test="${item.truck_length eq 31}">selected="selected"</c:if>>17米</option>
            		<option value="32" <c:if test="${item.truck_length eq 32}">selected="selected"</c:if>>17.5米</option>
            		<option value="33" <c:if test="${item.truck_length eq 33}">selected="selected"</c:if>>18米</option>
            		<option value="0" <c:if test="${item.truck_length eq 0}">selected="selected"</c:if>>其他</option>
            	</select>
            	<select id="truck_type" name="model.truck_type">
            		<option value="1" <c:if test="${item.truck_type eq 1}">selected="selected"</c:if>>普通单车</option>
            		<option value="2" <c:if test="${item.truck_type eq 2}">selected="selected"</c:if>>普通半挂</option>
            		<option value="3" <c:if test="${item.truck_type eq 3}">selected="selected"</c:if>>冷藏车</option>
            		<option value="4" <c:if test="${item.truck_type eq 4}">selected="selected"</c:if>>侧帘车</option>
            		<option value="5" <c:if test="${item.truck_type eq 5}">selected="selected"</c:if>>双飞翼车</option>
            		<option value="6" <c:if test="${item.truck_type eq 6}">selected="selected"</c:if>>改型车</option>
            		<option value="7" <c:if test="${item.truck_type eq 7}">selected="selected"</c:if>>罐式车</option>
            		<option value="8" <c:if test="${item.truck_type eq 8}">selected="selected"</c:if>>爬梯车</option>
            		<option value="9" <c:if test="${item.truck_type eq 9}">selected="selected"</c:if>>特种车辆</option>
            		<option value="10" <c:if test="${item.truck_type eq 10}">selected="selected"</c:if>>危险品车</option>
            		<option value="11" <c:if test="${item.truck_type eq 11}">selected="selected"</c:if>>笼子车</option>
            		<option value="12" <c:if test="${item.truck_type eq 12}">selected="selected"</c:if>>大件车</option>
            		<option value="13" <c:if test="${item.truck_type eq 13}">selected="selected"</c:if>>框架车</option>
            	</select>
            	<div class="zc_txt">
            	<span>期望运价：</span><input type="text" name="model.truck_freight" value="${order.truck_freight}" onkeyup="this.value=this.value.replace(/\D/g,'')"  onafterpaste="this.value=this.value.replace(/\D/g,'')"/><b>(元),</b><strong>如果运价不填写则默认为面议</strong>
            	</div>
            	<div class="zc_txt"><span class="benner-translateo-span">说明：</span>
            	
            	<textarea id="trunk_remark" name="model.trunk_remark"> ${order.trunk_remark}</textarea></div>
            </div>
