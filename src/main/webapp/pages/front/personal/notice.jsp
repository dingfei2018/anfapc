...
<html>
	<head>
		<meta charset="UTF-8">
		<title>公告管理</title>
		<link rel="stylesheet" href="${ctx}/static/pc/Personal/css/ncement.css?v=${version}"/>
	...
	<div class="content">
		<%@ include file="common/left.jsp"%>
			<div class="content-right">
			   <ul>
				   <li style="margin-right: 40px;"><a href="${ctx}/cpy/admin/notice">新增公告</a></li>
				   <li><a class="active"class="active" href="${ctx}/cpy/admin/noticeList">公告管理</a></li>
				</ul>
			
				<div class="content-right-top">
					<%-- <ul>
						<li><a href="${ctx}/cpy/admin/notice">新增公告</a></li>
						<li  class="active"><a href="${ctx}/cpy/admin/noticeList">公告管理</a></li>
					</ul> --%>
					    ...
					    <tr>
					     ...
					     <c:forEach items="${page.list}" var="n" varStatus="v">
						    <tr>
						       ...
						       <td><fmt:formatDate value="${n.valided}" type="date"/></td>
						       <td><a id="gtn" href="${ctx}/cpy/admin/notice?id=${n.id}">修改</a><a id="gtns" href="javascript:void(0);"  onclick="deleteNew(new Array('${n.id}'))">删除</a></td>
						    </tr>
					    </c:forEach>
				...
				Anfa.ajax({
					...
					success : function(data) {
						if (data.success==true) {
							...
							window.location.href="${ctx}/cpy/admin/noticeList?pageNo="+curr;
						} else {
							layer.msg(data.msg);
						}
					}
		...
		function appendHtml(ins){
			var html ="<tr><td><input type=\"checkbox\" value='"+ins.id+"'/> 1</td>"
			       +"<td>"+ins.content+"</td><td>"+ins.created+"</td>"
			       +"<th>"+ins.valided+"</th>"
			       +"<td><a id=\"btn\" href='${ctx}/cpy/admin/notice?id="+ins.id+"'>修改</a><a id=\"btns\" href=\"javascript:void(0);\"  onclick=\"deleteNew(new Array('"+ins.id+"'))\">删除</a></td>";
		       html +="</tr>";
		       return html;
		}
