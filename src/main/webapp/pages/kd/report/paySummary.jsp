       ...
       <%@ include file="../common/head.jsp" %>
		...
		<div class="banner">
			...
			<form onsubmit="return false;">
				<ul>
					...
					<li>
						<a href="${ctx }/kd/report/paySummary" class="active at" >应付汇总表</a>
					</li>
					<li>
				...
				<div class="banner-right-list2">
					<p class="banner-right-p">应付汇总表</p>
		...
		<%@ include file="../common/loginfoot.jsp" %>
				...
				function init(){
					...
					$.ajax({
						type:'post',
						url:'${ctx}/kd/report/getPaySummaryData',
						dataType : "json",
						data:{"startTime":$('#startTime').val(),"endTime":$('#endTime').val(),"networkId":$('#netWorkId').val()},
						success:function(data){
							layer.close(index);
								var transferfee=new Array();
								var loadfee=new Array();
								var netPayfee=new Array();
								var content="";
								var th='<thead><th>月份</th><th>配载车次</th><th>配载应付</th><th>中转单量</th><th>中转应付</th><th>网点对账应付</th></thead>';
								for(var i=0;i<data.length;i++){
								var row=data[i];
								if(row.time!='合计'){
									transferfee.push(row.transferfee);			
									loadfee.push(row.loadFee);
									netPayfee.push(row.netPayfee);
									year.push(row.time);
								}
								...
								if(i==data.length-2){
								 ...
								 content+='<tr><td>'+row.time+'</td>'+
											'<td>'+row.countLoad+'</td>'+
											'<td style="color: #ff7801;">'+row.loadFee+'</td>'+
											'<td>'+row.transferCount+'</td>'+
											'<td style="color: #ff7801;">'+row.transferfee+'</td>'+
											'<td style="color: #ff7801;">'+row.netPayfee+'</td>'+
											'</tr>'	; 
								}
				...
				function downExcel(){
					...
					layer.confirm(
							...
							function() {
								window.location.href='${ctx}/kd/report/exportPaySummary?startTime='+$('#startTime').val()+'&endTime='+$('#endTime').val()+'&networkId='+$('#netWorkId').val();
					}, function() {
							});
