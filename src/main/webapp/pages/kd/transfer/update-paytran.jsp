...
<html>
	...
	<body>
		<div class="gloup">
			...
			<div class="gloup-list2">
				...
				<input type="hidden" name="shipTransfer.transfer_id" " value="${shipTransfer.transfer_id }"/>
				<span>中转方 : </span><input value="${shipTransfer.transferName }" disabled="disabled" />
				<span>中转单号 : </span><input type="text" name="shipTransfer.ship_transfer_sn" value="${shipTransfer.ship_transfer_sn }" disabled="disabled"/>
				<span>中转时间 : </span><input type="text" name="shipTransfer.ship_transfer_time" value="${shipTransfer.ship_transfer_time }" disabled="disabled" /><br />
				<span class="span">中转费 : </span><input id="y_fee" class="input" name="shipTransfer.ship_transfer_fee" value="${shipTransfer.ship_transfer_fee }" type="text" />
				</form>
			</div>
			
			<input class="button" type="button" value="确认修改" onclick="updateTransfer();"/>
			<input class="button" type="button" value="返回"  onclick="parent.layer.closeAll();"/>
		</div>
	</body>
	...
	function updateTransfer(){
		...
		if($("#y_fee").val().length==0){
			Anfa.show("请输入中转费","#y_fee");
			return;
		}else if(!numTest.test($("#y_fee").val())){
			Anfa.show("请输入非负数正确费用","#y_fee");
			return;
		}
		$('#transferForm').submit();
