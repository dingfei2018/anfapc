...
public class RoleController extends BaseProjectController {
	...
	public void list() {
		...
		String sqlSelect = "select t.* "
				+ ",(select group_concat(m.name) from sys_role_menu rm left JOIN  sys_menu m ON rm.menu_id = m.id where rm.role_id = t.id ) as menus ";
	...
	public void auth() {
		...
		String menus = new RoleSvc().getMemus(role_id);
	...
	public void auth_save() {
		...
		new RoleSvc().saveAuth(role_id, menus, getSessionSysUser().getUserId());
		renderMessage("保存成功");
	}
